...
class Snake:
    ...
    def reset(self):
        # The head is in the middle, with the tail to the left
        self.pieces = [[self.width // 2 - 2, self.height // 2],
    ...
    def step(self, action, n_games):
        ...
        if np.array_equal(action, [1, 0, 0]):
            # Go straight, direction does not change
            pass
        elif np.array_equal(action, [0, 1, 0]):
            # Turn left
            self.direction = (self.direction + 3) % 4
            pass
        else: # When action is [0, 0, 1]
            # Turn right
    ...
    def update_ui(self, n_games):
        ...
        # Score
        text = self.font.render("Game: {}, Score: {}".format(n_games, self.score), True, self.fontColor)
